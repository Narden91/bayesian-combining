...
def run_analysis(run, global_seed, root_output_folder, sample_df, cfg, num_tasks, analysis_type, verbose, debug, file_list, file_list_comb=None):
    ...
    if cfg.experiment.stacking_method == 'Bayesian':
        ...
        logging.info(f"Weighted Majority Vote, using first level predictions, metrics: \n {wmv_metrics}") if verbose else None
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                logging.error(f"Run raised an exception: {e}")
